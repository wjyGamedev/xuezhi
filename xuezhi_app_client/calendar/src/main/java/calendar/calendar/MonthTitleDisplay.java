...
public class MonthTitleDisplay
{
	...
	public MonthTitleDisplay(TextView title)
	{
		this.title = title;
		...
		yTranslate = (int)TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, DEFAULT_Y_TRANSLATION_DP, res.getDisplayMetrics()
												   );
	}
	...
	private void doChange(final long now, final CalendarDay currentMonth, boolean animate)
	{
		...
		else
		{
			...
			title.animate().translationY(yTranslation * -1).alpha(0).setDuration(animDuration).setInterpolator(interpolator).setListener(new AnimatorListener()
																																		 {

																																			 @Override
																																			 public void onAnimationCancel(Animator animator)
																																			 {
																																				 title.setTranslationY(
																																						 0
																																									  );
																																				 title.setAlpha(
																																						 1
