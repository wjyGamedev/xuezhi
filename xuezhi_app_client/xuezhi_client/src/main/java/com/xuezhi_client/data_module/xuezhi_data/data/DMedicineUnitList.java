...
public class DMedicineUnitList
{
	private int                      m_Status       = ProtocalConfig.HTTP_OK;
	private ArrayList<DMedicineUnit> m_medicalUnits = new ArrayList<>();
	...
	public synchronized boolean serialization(JSONObject response) throws JSONException
	{
		...
		//03. 序列化json
		JSONArray jsonArray = response.getJSONArray(MedicineUnitConfig.LIST);
		if (jsonArray == null)
		{
			throw new JsonSerializationException(NET_ERROR_JSON_SERILIZATION + ":" + MedicineUnitConfig.LIST);
		}

		JSONObject    jsonObject  = null;
		DMedicineUnit medicalUnit = null;
		for (int index = 0; index < jsonArray.length(); index++)
		{
			jsonObject = (JSONObject)jsonArray.get(index);
			medicalUnit = new DMedicineUnit();
	...
	public synchronized ArrayList<DMedicineUnit> getMedicalUnits()
	{
		return m_medicalUnits;
	}

	public synchronized DMedicineUnit getMedicalUnitByID(int id)
	{
		for (DMedicineUnit medicalUnit : m_medicalUnits)
		{
