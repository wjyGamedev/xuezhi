...
public class DMedicalStockList
{
	private int                 m_Status   = ProtocalConfig.HTTP_OK;
	private ArrayList<DMedicalStock> m_medicalStocks = new ArrayList<>();
	...
	public synchronized boolean serialization(JSONObject response) throws JSONException, ParseException
	{
		...
		DMedicalStock medicalStock = null;
		for (int index = 0; index < jsonArray.length(); index++)
		{
			jsonObject=(JSONObject)jsonArray.get(index);
			medicalStock = new DMedicalStock();
	...
	public synchronized ArrayList<DMedicalStock> getMedicalStocks()
	{
		return m_medicalStocks;
	}

	public synchronized DMedicalStock getMedicalByID(int id)
	{
		for (DMedicalStock medicalStock : m_medicalStocks)
		{
