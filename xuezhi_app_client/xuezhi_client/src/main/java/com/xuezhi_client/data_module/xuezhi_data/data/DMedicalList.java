...
public class DMedicalList
{
	private int                 m_Status    = ProtocalConfig.HTTP_OK;
	private ArrayList<DMedical> m_medicals = new ArrayList<>();
	...
	public synchronized boolean serialization(JSONObject response) throws JSONException
	{
		...
		DMedical hospital = null;
		for (int index = 0; index < jsonArray.length(); index++)
		{
			jsonObject=(JSONObject)jsonArray.get(index);
			hospital = new DMedical();
	...
	public synchronized ArrayList<DMedical> getMedicals()
	{
		return m_medicals;
	}

	public synchronized DMedical getMedicalByID(int id)
	{
		for (DMedical medical : m_medicals)
		{
			if (medical.getID() == id)
			{
				return medical;
