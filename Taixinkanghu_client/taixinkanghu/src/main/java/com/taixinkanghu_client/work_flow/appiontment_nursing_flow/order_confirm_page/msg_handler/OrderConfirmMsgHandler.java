...
package com.taixinkanghu_client.work_flow.appiontment_nursing_flow.order_confirm_page.msg_handler;
...
import com.taixinkanghu_client.work_flow.appiontment_nursing_flow.nurse_order_pay.ui.NurseOrderPayInNormalActivity;
import com.taixinkanghu_client.work_flow.appiontment_nursing_flow.order_confirm_page.ui.OrderConfirmActivity;
import com.taixinkanghu_client.work_flow.appiontment_nursing_flow.order_confirm_page.ui.SelectPatientStateFragment;
import com.taixinkanghu_client.work_flow.appiontment_nursing_flow.patient_info_page.ui.PatientActivity;
import com.taixinkanghu_client.work_flow.appiontment_nursing_flow.user_protocal_page.UserProtocalActivity;

public class OrderConfirmMsgHandler extends BaseAppiontmentNursingFlowUIMsgHandler
{
	...
	{
		RequestNurseOrderConfirmInNormalEvent event = m_dAppiontmentNursingFlow.constructRequestNurseOrderConfirmInNormalEvent();
	...
	public void onEventMainThread(AnswerNurseOrderConfirmInNormalEvent event)
	{
		...
		m_dAppiontmentNursingFlow.setOrderID(event.getOrderID());
		m_dAppiontmentNursingFlow.setOrderSerialNum(event.getOrderSerialNum());
		m_dAppiontmentNursingFlow.setTotalPrice(event.getTotalPrice());

		//02. 跳转到支付页面
		Intent intent = new Intent(orderConfirmActivity, NurseOrderPayInNormalActivity.class);
	...
	public void updateContent()
	{
		...
		int headerImgResID = m_dAppiontmentNursingFlow.getNurseHeaderImgResID();
		orderConfirmActivity.getNurseHeadImgIV().setImageResource(headerImgResID);

		String name = m_dAppiontmentNursingFlow.getNurseName();
		orderConfirmActivity.getNameTV().setText(name);

		String jobNum = m_dAppiontmentNursingFlow.getNurseJobNum();
		orderConfirmActivity.getJobNumTV().setText(jobNum);

		String nursingLevel = m_dAppiontmentNursingFlow.getNursingLevel();
		orderConfirmActivity.getNuringLevelTV().setText(nursingLevel);

		String serviceDate = m_dAppiontmentNursingFlow.getServiceDate();
		orderConfirmActivity.getServiceDateTV().setText(serviceDate);

		String serviceAddress = m_dAppiontmentNursingFlow.getServiceAddress();
		orderConfirmActivity.getServiceAddressTV().setText(serviceAddress);

		//被护理人
		String patientName = m_dAppiontmentNursingFlow.getPatientName();
	...
	public void updatePrice()
	{
		...
		//金额
		int allNum       = m_dAppiontmentNursingFlow.getAllNum();
		int chargePerAll = m_dAppiontmentNursingFlow.getChargePerAll();
		if (allNum != 0)
		{
		...
		int dayNum       = m_dAppiontmentNursingFlow.getDayNum();
		int chargePerDay = m_dAppiontmentNursingFlow.getChargePerDay();
		if (dayNum != 0)
		{
		...
		int nightNum       = m_dAppiontmentNursingFlow.getNightNum();
		int chargePerNight = m_dAppiontmentNursingFlow.getChargePerNight();
		if (nightNum != 0)
		{
		...
		int totalCharge = m_dAppiontmentNursingFlow.getTotalChargeDisplay();
